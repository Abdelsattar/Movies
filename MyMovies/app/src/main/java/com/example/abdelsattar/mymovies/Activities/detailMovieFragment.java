...
import com.example.abdelsattar.mymovies.Adapters.ExpandableListAdapter;
import com.example.abdelsattar.mymovies.Objects.Movie;
import com.example.abdelsattar.mymovies.R;
import com.example.abdelsattar.mymovies.Objects.Review;
import com.example.abdelsattar.mymovies.Objects.Video;
...
public class detailMovieFragment extends Fragment implements  View.OnClickListener{
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        if(bundle !=null){
            ...
            movieObj.setReleaseDate(bundle.getString("rDate"));
    ...
    private void shareLink(String share) {
...
//                    editor.clear();
//                    editor.commit();
                int num = pref.getInt(getString(R.string.number_Fav_Movies), 0);

        boolean exist = pref.getBoolean(movieObj.getMovieID(), false);
        String movies = pref.getString(getString(R.string.pref_movie_key), "");
        if(!exist) {
            num++;
//            editor.putString(Integer.toString(num), movieObj.getMovieID());
//            editor.putInt(getString(R.string.number_Fav_Movies), num);
            editor.putBoolean(movieObj.getMovieID(),true);
            if(movies !=null) {
                editor.putString(getString(R.string.pref_movie_key),
                        movies + movieObj.encode() + "#");
            } else {
                editor.putString(getString(R.string.pref_movie_key),
                        movieObj.encode() + "#");
            }
